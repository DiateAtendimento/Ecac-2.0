...
/**
 * Definição de intenções: cada uma com padrões (regex) e respostas possíveis.
 */
const intents = [
  {
    name: 'saudacao',
  ...
  },
  {
    ...
    patterns: [
      /\becac\b/i,
      /centro virtual/i,
      /como (?:funciona|opera|utiliza)\s+ecac/i
    ],
    responses: [
  ...
  },
  {
    ...
    patterns: [
      /regra de\s*(?:a?bat(?:imento)?|batimento)\s*0?9\b/i,
      /duplicidade/i,
  ...
  },
  {
    ...
    patterns: [
      /regra de\s*(?:batimento|abatimento)\s*0?8\b/i,
      /comprova(?:ção|cao)/i,
      /deduzidos?/i,
      /compensados?/i
    ],
  ...
  },
  {
    ...
    patterns: [
      /regra de\s*(?:batimento|abatimento)\s*20\b/i,
      /benef[ií]cios?/i,
      /etapa\s*3/i,
      /transfer[êe]ncia/i
    ],
  ...
  },
  {
    ...
    name: 'gescon_cadunico',
        ...
        responses: [
            'O Gescon gerencia benefícios previdenciários dos servidores (RPPS), enquanto o CADÚNICO é para programas sociais (familias em vulnerabilidade). Para dúvidas do CADÚNICO, acesse https://www.gov.br/mds/pt-br/acesso-a-informacao/participacao-social/fale-conosco ou ligue 121, ou use WhatsApp no número 61 4042-1552.'
        ]
    },
    {
    ...
    },
    {
        ...
        responses: [
            'Para impugnar NAF/PAP: instrua a impugnação com prova de representação legal e apresente o documento original à SPREV por protocolo in loco ou eletrônico, ou via postal (caput do art. 257 da Port. 1.467/2022) ao endereço da CGAUC (Esplanada dos Ministérios, Bloco F, Anexo A, Sala 427, CEP 70059-900, Brasília/DF). O ente tem 30 dias, sem ficar irregular durante a análise; se mantida a irregularidade, instaura-se PAP. Envie justificativas de regularização apenas por peticionamento eletrônico ou postal (não via Gescon). O auditor emitirá o Despacho de Justificativa (DJ). Em caso de recurso, o ente terá mais 30 dias para apresentá-lo.'
        ]
    },
    {
        name: 'pap_naf_copia',
...
/**
 * Fallback (quando nenhuma intenção casar).
 */
...
/**
 * Função principal: recebe o texto do usuário e retorna uma resposta apropriada.
 */
function getResponse(text) {
  ...
  // Se nada casar, retorna fallback
  return fallbackResponses[Math.floor(Math.random() * fallbackResponses.length)];
}

// Exporta a função para uso no chatbot.js
// Em browsers sem módulos, basta garantir que getResponse fique no escopo global.
